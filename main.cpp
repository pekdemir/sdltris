...
void castShadow()
{
	...
	forij
	{
		if(currPiece[i][j] == B)
		{
			...
			SDL_FillRect(screen,&rect,0x33333333);
		}
	}
	//reset piece pos
	pieceY = temp;
...
void drawPiece()
{
	...
	forij
	{
		if(currPiece[i][j] == B)
		{
			...
			rect.y = (j+pieceY)*BLOCKSIZE;
			rect.w = rect.h = BLOCKSIZE;
			SDL_FillRect(screen,&rect,currColor);
		}
	}
	//nextPiece
	forij
	{
		if(nextPiece[i][j] == B)
		{
			...
			rect.y = (j)*BLOCKSIZE;
			rect.w = rect.h = BLOCKSIZE;
			SDL_FillRect(screen,&rect,nextColor);
		}
	}
}
...
void drawStack()
{
	for(int j=0;j<H;j++)
	{
		for(int i=0;i<W;i++)
		{
			...
			rect.y = (j)*BLOCKSIZE;
			rect.w = rect.h = BLOCKSIZE;
			SDL_FillRect(screen,&rect,map[i][j]);
		}
	}
}
...
void drawAll()
{
	...
	SDL_FillRect(screen,NULL,0x99999999);
...
void initSDL()
{
	...
	screen = SDL_SetVideoMode(WIDTH, HEIGHT, 32, /*SDL_NOFRAME |*/ SDL_DOUBLEBUF);
}
